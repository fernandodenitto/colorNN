...
% colonne sono pari al DOPPIO del numero di ripetizioni N. Nelle prime N ci
% saranno per ogni ripetizione in coefficiente di regressione della i-esima
% iterazione. Nelle seconde N invece ci sarà il tempo di esecuzione del
...
tabellaRiepilogativa=zeros(maxHiddenNeurons,1+(numRipetizioni*2));

tabellaRiepilogativaPerc=zeros(maxHiddenNeurons,3);
...
for numHiddenNeurons=1:maxHiddenNeurons
    ...
    tic
    tabellaRiepilogativa(numHiddenNeurons,1+j)=performNN(masterCopyFeaturesSel, deltaE,'trainlm',numHiddenNeurons);
    tabellaRiepilogativa(numHiddenNeurons,1+numRipetizioni+j)=toc;
